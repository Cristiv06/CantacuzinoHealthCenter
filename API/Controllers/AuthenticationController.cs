...
namespace API.Controllers
{
    ...
    public class AuthenticationController: ControllerBase
    {
        ...
        public async Task<IActionResult?> Authenticate(AuthCredentialsDTO authCredentials)
        {
            if (authCredentials.AppId == AppSettings.AuthSettings.UIClientId && authCredentials.AppKey == AppSettings.AuthSettings.UIClientSecret)
            {
                ...
                if(user == null)
                {
                    return new UnprocessableEntityObjectResult(new { errors = "Credentials not good. Try again!!!!"});
                }

                var jwt_token = AuthExtensions.BuildJWT(AppSettings.AuthSettings.Issuer, AppSettings.AuthSettings.SymmetricSecurityKey, user);
                return Ok(new AuthResponseDTO { jwt = jwt_token, userDetails = user });
            }
            else
            {
                return null;
