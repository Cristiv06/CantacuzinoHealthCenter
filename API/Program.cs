...
builder.Services.AddDbContext<CantacuzinoHealthCenterContext>(
...
//{
    app.UseSwagger();
    app.UseSwaggerUI();
if (app.Environment.IsDevelopment() || app.Environment.IsProduction()) // Add IsProduction() for temporary debugging
{
...
//app.UseCors(c => c.WithOrigins(AppSettings.AuthSettings.UIAppUrl).AllowAnyHeader().AllowAnyMethod());
app.UseCors(c =>
{
    c.AllowAnyHeader().AllowAnyMethod().AllowCredentials().WithOrigins(AppSettings.AuthSettings.UIAppUrl);
});
