    ...
    public class APIEndpointAttribute : ActionFilterAttribute, IOrderedFilter
    {
        ...
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            if (context.ModelState.IsValid
                ...
                && context.Result is not OkObjectResult
                && context.Result is not CreatedResult
                && (context.Filters.Count(f => f is APIEndpointAttribute) == 1 || Order == 0))
            {
                var resultObj = SendRawResult ? context.Result : (context.Result as ObjectResult)!.Value;
